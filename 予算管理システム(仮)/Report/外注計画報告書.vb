...
Public Class 外注計画報告書
    Public Function ReportLoad() As String
        ...
        Dim field16 As Field
        field16 = CType(レポート.C1FlexReport1.Fields("特記事項"), Field)
            ...
            For VendorLoop As Integer = 0 To VendorCount - 1
                If VendorIDList.Count > VendorLoop Then
                    ...
                    If Not AmountReader = 0 Then
                        DT.Rows(VendorLoop).Item("amount" & OutsrcNoLoop) = AmountReader
                        DT.AcceptChanges()
                    Else
                        DT.Rows(VendorLoop).Item("amount" & OutsrcNoLoop) = 0
                        DT.AcceptChanges()
                    End If
                End If
            Next
